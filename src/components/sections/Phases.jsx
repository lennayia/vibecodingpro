...
// PhaseCard sub-component - memoized for performance
const PhaseCard = memo(({ phase, globalIndex, localIndex, showArrow, isLastInPackage, phasesLength }) => {
  return (
    <div key={phase.number} className="relative">
      ...
      <div className={`w-full min-[600px]:w-5/6 min-[1200px]:w-2/3 mx-auto ${globalIndex % 2 === 0 ? 'min-[600px]:ml-0 min-[600px]:mr-auto' : 'min-[600px]:ml-auto min-[600px]:mr-0'}`}>
        ...
        >
          ...
          <motion.div
            className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 holographic-hover-glow"
          />

          {/* Header */}
          <div className="flex items-start gap-3 relative z-10 mb-4 mb-fluid-phases-xs">
      ...
      {showArrow && !isLastInPackage && (
        <div className={`w-5/6 min-[1200px]:w-2/3 mx-auto ${globalIndex % 2 === 0 ? 'ml-0 mr-auto' : 'ml-auto mr-0'}`}>
          <div className={`flex ${globalIndex % 2 === 0 ? 'justify-start pl-20 min-[1200px]:pl-56' : 'justify-end pr-40 min-[1200px]:pr-64'} py-2`}>
            ...
            >
              <ArrowDown className={`w-8 h-8 text-accent ${globalIndex % 2 === 0 ? '-rotate-45' : 'rotate-45'}`} strokeWidth={2} />
            </motion.div>
          </div>
        </div>
      )}
...
// PackageHeader sub-component - memoized for performance
const PackageHeader = memo(({ packageName, phasesCount, isExpanded, onToggle }) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: -10 }}
      whileInView={{ opacity: 1, y: 0 }}
...
function Phases() {
  ...
  return (
    <Section id="process" background="light" centered={true} customPadding="pt-12 pb-12 md:pt-4 md:pb-8 lg:pt-8 lg:pb-16 px-[4%]" showScrollIndicator={true}>
      <div className="w-full">
        <motion.div {...fadeIn}>
          ...
          <div className="max-w-5xl mx-auto relative px-4 min-[600px]:px-12 min-[1200px]:px-16">
            <div className="flex flex-col gap-8 gap-fluid-phases-lg mt-8 mt-fluid-phases-lg">
                  ...
                  >
                    ...
                    <AnimatePresence initial={false}>
                      {isExpanded && (
                        ...
                        >
                          <div className="flex flex-col gap-4 gap-fluid-phases-xs">
                            {pkg.phases.map((phase, localIndex) => {
                              ...
                              return (
                                <PhaseCard
                                  ...
                                  isLastInPackage={localIndex >= pkg.phases.length - 1}
                                  phasesLength={pkg.phases.length}
                                />
                              )
                            })}
                          </div>
